...
# 5. Математически технически анализ
...
# =========================================================
# ГЛАВЕН ЕКРАН - КОМПАКТНА СТРУКТУРА НА ЕДИН ЕКРАН (БЕЗ СКРОЛ)
# =========================================================
...
# РЕД 2: Посока и Пазарно съотношение (Всичко е хоризонтално сбито)
col_trend, col_ratio = st.columns(2)

with col_trend:
    st.markdown("**🎯 Направление на пазара:**")
    # Използване на официални Streamlit контейнери със защитени икони
    if is_low_volatility:
        buy_ratio = random.randint(49, 51)
        with st.status("⚠️ НИСКА ВОЛАТИЛНОСТ", state="warning"):
            st.write("↔️ Странично движение (Рейндж). Липсва мощност.")
    elif is_intertwined:
        buy_ratio = random.randint(46, 54)
        with st.status("🟠 ФАЛШИВ ПРОБИВ", state="warning"):
            st.write("🔄 Линиите се преплитат. Пазарът е нестабилен.")
        ...
        if current_p >= emaFast_p and is_strong_momentum and fast_ema_slope > 0:
            ...
            with st.status("🚀 СИЛЕН КУПУВАЧ (STRONG BUY)", state="complete"):
                st.write("📈 Изразен бичи тренд. Цената расте ускорено.")
        else:
            buy_ratio = random.randint(58, 68)
            with st.status("📈 СЛАБ КУПУВАЧ (WEAK BUY)", state="complete"):
                st.write("⏳ Корекция във възходящия тренд. Инерцията отслабва.")
    elif emaFast_p < emaMid_p < emaSlow_p:
        if current_p <= emaFast_p and is_strong_momentum and fast_ema_slope < 0:
            buy_ratio = 100 - random.randint(88, 98)
            with st.status("🚨 СИЛЕН ПРОДАВАЧ (STRONG SELL)", state="error"):
                st.write("📉 Изразен мечи спад. Цената пада интензивно.")
        else:
            buy_ratio = 100 - random.randint(60, 70)
            with st.status("📉 СЛАБ ПРОДАВАЧ (WEAK SELL)", state="error"):
                st.write("⏳ Лека корекция. Липсва агресивен натиск надолу.")
    else:
        buy_ratio = random.randint(47, 53)
        with st.status("➖ КОНСОЛИДАЦИЯ", state="warning"):
            st.write("⚪ Няма ясен сигнал от индикаторите в момента.")

with col_ratio:
    st.markdown(f"**📊 Пазарни Сили ({timeframe_label}):**")
    # Компактно разпределение на силите
